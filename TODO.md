...
* [ ] Fix CI so that it indeed runs helm lint against generated templates
   ...
   * Would require to attach a workspace and persist generated files into it
   * However the lint job from the orbs isn't supporting definining workspace for dynamically generated charts
   * Inline the lint orbs job ? 
...
* [ ] use this repo to generate helm chart for a service broker in Orange paas-templates
   ...
   * [x] Update the catalog to use one useful for our demos:
      * [x] Osb-cmdb-0
        * [x] Make a copy with Redact urls
      * [x] Developer tools
   * [ ] generate charts for each catalog in distinct directories  
   * [ ] Commit generated charts to get them linted by circle ci ?  
   * [ ] Commit generated charts in 
      * ~~branch only published internally: `orange-private-catalog`~~
      * distinct repo only published internally: `osb-2-helm-orange-private-catalog`
    ...
    * Implementation alternatives
        ...
        * Plain go templates https://godoc.org/text/template
            * [~] with a CLI https://github.com/hairyhenderson/gomplate
               * [x] Install the tool
               * [x] Craft a template
               * [x] Manually assert result against sample-p-mysql-service-definition.json
               * [ ] Package 
               * [~] Harden with conditionals for optional teals  
               * [ ] Assert unit test assertion for each case
